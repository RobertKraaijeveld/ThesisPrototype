...
namespace ThesisPrototype
{
    ...
    public class SensorValuesRow : AbstractModel
    {
        // TODO: TOO COMPLICATED, FIX THIS
        public static SensorValuesRow CreateFromJson(string jsonRow)
        {
            var asDictionary = JsonConvert.DeserializeObject<Dictionary<string, object>>(jsonRow);
            var basicDeserializedModel = JsonConvert.DeserializeObject<SensorValuesRow>(jsonRow);
            
            // We need to convert the human-readable TS to unix ts, so we use the dictionary value for the 'ts' column.
            var originalTimestamp = DateTime.Parse(asDictionary["ts"].ToString());
            var asUnixTimestamp = (Int32)(originalTimestamp.Subtract(new DateTime(1970, 1, 1))).TotalSeconds;

            basicDeserializedModel.UnixTs = asUnixTimestamp;
            return basicDeserializedModel;
        }
